...
export const NavigationProvider = ({ children }) => {
  ...
  React.useEffect(() => {
    const loadSettings = async () => {
      try {
        ...
        const savedMethod = await AsyncStorage.getItem('@prayer:calculationMethod');
        if (savedMethod) {
          setCalculationMethodState(savedMethod);
        }
      } catch (error) {
  ...
  const setCalculationMethod = useCallback(async (mode) => {
    try {
      setCalculationMethodState(mode);
      await AsyncStorage.setItem('@prayer:calculationMethod', mode);
    } catch (error) {
