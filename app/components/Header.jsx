...
import { getMethodDisplayName } from '../utils/calculationMethodDetector';
...
// ─── Flash Icon with Premium Hint ────────────────────────────────────────────
const FlashIconWithHint = ({ onPress, style, showHint }) => {
  ...
  return (
    <TouchableOpacity onPress={onPress} style={[style, { position: 'relative' }]}>
      {showHint && (
        <Animated.View
          style={{
            position: 'absolute',
...
const Header = ({
  ...
  const handleOptionPress = (option) => {
    ...
    if (option.isModal) {
      setTimeout(() => {
        ...
        else if (option.modalType === 'calculationMethod') setCalculationMethodModalVisible(true);
      }, 300);
    }
  };
  ...
  return (
    <>
          ...
          <View style={styles.right}>
            ...
            {showSearchIcon && (
              <TouchableOpacity
                ...
                style={styles.icon}
              >
                <Ionicons name="search-outline" size={ms(23.6)} color="#3b82f6" />
              </TouchableOpacity>
            )}
            {showFlashIcon && (
              <FlashIconWithHint onPress={onFlashPress} style={styles.icon} showHint={showFlashHint} />
            )}
            ...
            <TouchableOpacity
              ...
              style={[styles.icon, styles.headerIcon]}
            >
              <Ionicons name="grid-outline" size={ms(21.6)} color="#3b82f6" />
            </TouchableOpacity>
          </View>
        </ThemedView>
      </View>
      ...
      />}
      {zakatModalVisible && <ZakatModal
        visible={zakatModalVisible}
        onClose={() => setZakatModalVisible(false)}
      />}
      {calculationMethodModalVisible && <CalculationMethodModal
        visible={calculationMethodModalVisible}
        onClose={() => setCalculationMethodModalVisible(false)}
        theme={theme}
        currentMethod={calculationMethod}
        onSelectMethod={setCalculationMethod}
      />}
            ...
            >
              <View style={styles.optionsGrid}>
                {settingsOptions.map((option) => {
                  ...
                  const isLocked = option.isThemeToggle && !isPremium;
                  return (
                    <TouchableOpacity
                      key={option.id}
                      style={[
                    ...
                    >
                      ...
                      {isLocked && (
                        <View style={{ position: 'absolute', top: ms(12), right: ms(12) }}>
                          <Ionicons
                            name="lock-closed"
                            size={ms(16)}
                            color={theme.text}
