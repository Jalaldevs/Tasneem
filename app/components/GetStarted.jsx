...
// ─── PREMIUM PAYWALL ─────────────────────────────────────────────────────────
const PremiumPaywall = ({ onClose, onSubscribe, productPrice = '$3.99', onRestore, dbReady, isDownloading, progress }) => {
  const { t } = useAppTranslation();
  ...
  const FEATURES = [
    ...
    { icon: 'weather-night', label: t('premium.features.darkModeTitle') },
    { icon: 'lightning-bolt', label: t('premium.features.dailyInsightTitle') },
  ];

  if (Platform.OS === 'ios') {
    FEATURES.splice(1, 0, { icon: 'book-open-page-variant-outline', label: t('premium.features.bookmarksTitle') });
  }
  ...
  return (
    <>
      ...
      <Animated.View style={[
        ...
        {/* CTA */}
        <TouchableOpacity
          style={[paywallStyles.ctaWrap, isDownloading && { opacity: 0.6 }]}
          onPress={() => {
            if (!accepted) return;
            if (!dbReady && !isDownloading) {
              downloadDatabase();
              return;
            }
            if (dbReady && !isDownloading) {
              onSubscribe?.(SUBSCRIPTION_SKU_MONTHLY);
            }
          }}
          activeOpacity={0.87}
        ...
        >
          <LinearGradient
            colors={isDownloading ? ['#64748b', '#475569', '#334155'] : ['#1e4f78', '#1b83de', '#1b83de']}
            start={{ x: 0, y: 0 }}
            end={{ x: 1, y: 0 }}
            style={paywallStyles.ctaGradient}
          >
            <View style={paywallStyles.ctaInner}>
              <Text style={paywallStyles.ctaText}>
                {isDownloading
                  ? t('premium.downloadingTitle', 'Downloading Data...')
                  : (!dbReady ? t('premium.downloadAssets', 'Download Assets First') : t('premium.trialCta'))}
              </Text>
              <Text style={paywallStyles.ctaSubtext}>
                {isDownloading
                  ? `${(progress * 100).toFixed(0)}%`
                  : (!dbReady ? t('premium.downloadAssetsSub', 'Required for premium features') : t('premium.trialCtaSub'))}
              </Text>
            </View>
            <Ionicons name={isDownloading ? "hourglass-outline" : (!dbReady ? "download-outline" : "arrow-forward")} size={18} color="#e8f2fb" style={{ marginLeft: 8 }} />
          </LinearGradient>
        </TouchableOpacity>
...
const GetStarted = ({ onClose, onPressGetStarted, onRestore }) => {
  ...
  const productPrice = loading ? t('premium.loading') : monthlyPrice;
