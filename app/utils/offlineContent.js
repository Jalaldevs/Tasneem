...
import catalog from '../constants/editions.json';
...
export const readOfflineSunnahEdition = async (editionKey) => {
  ...
  try {
    const rows = await db.getAllAsync(`SELECT * FROM Hadiths WHERE edition = ? ORDER BY hadithnumber ASC`, [editionKey]);
    ...
    const hadiths = rows.map(row => {
      let bookKey = '1';
      if (row.reference) {
        const match = row.reference.match(/"book"\s*:\s*([^,}]+)/);
        if (match) bookKey = String(match[1]).replace(/["']/g, '').trim();
      }
      
      if (!metadata.sections[bookKey]) {
        metadata.sections[bookKey] = `Chapter ${bookKey}`;
        metadata.section_details[bookKey] = {
          hadithnumber_first: row.hadithnumber,
          hadithnumber_last: row.hadithnumber,
          arabicnumber_first: row.arabicnumber,
          arabicnumber_last: row.arabicnumber
        };
      } else {
        metadata.section_details[bookKey].hadithnumber_last = row.hadithnumber;
        metadata.section_details[bookKey].arabicnumber_last = row.arabicnumber;
      }
      ...
      return {
        ...
        get grades() {
          if (!parsedGrades) {
            try {
              parsedGrades = JSON.parse(row.grades || '[]');
            } catch {
              parsedGrades = [];
            }
          }
        ...
        get reference() {
          if (!parsedRef) {
            try {
              parsedRef = JSON.parse(row.reference || '{}');
            } catch {
              parsedRef = {};
            }
          }
