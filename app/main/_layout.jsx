...
import DownloadingAssetsModal from '../components/DownloadingAssetsModal';
...
export default function SwipePages() {
  ...
  useEffect(() => {
    ...
    checkDb();
  }, [isDownloading, showDownloadModal]);
  ...
  const handleTabAttempt = (index, isSwipe = false) => {
    if (index === 2 && !dbReady) {
  ...
  const onTabPress = (index) => {
    if (!handleTabAttempt(index, false)) return;
  ...
  const onPageSelected = (e) => {
    ...
    if (!handleTabAttempt(newIndex, true)) return;
