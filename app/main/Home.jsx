...
const calculatePrayerTimes = (latitude, longitude, date = new Date(), method = 'MuslimWorldLeague', offsets = {}) => {
  ...
  for (const [cap, low] of Object.entries(keys)) {
    if (offsets[cap]) {
      pt[low] = new Date(pt[low].getTime() + offsets[cap] * 60000);
    }
  }
...
// ─── MAIN COMPONENT ─────────────────────────────────────────────
const Home = () => {
  const { colorScheme: scheme, triggerPrayerAlert, dismissPrayerAlert, registerAthanControl, calculationMethod } =
  ...
  const handleSaveOffsets = async (newOffsets) => {
    ...
    // Recalculate immediately with new offsets
  ...
  const updateCountdown = () => {
    ...
    let nextFound = null;
    let nextFoundDate = null;

    for (let i = 0; i < order.length; i++) {
      const name = order[i];
      const lowName = lowerOrder[i];
      const d = prayerTimes._raw[lowName]; // The actual Date object

      if (d && d > now) {
        nextFound = { name, time: prayerTimes[name] };
        nextFoundDate = d;
        // The current prayer is the one before this one (excluding Sunrise if we want)
        for (let j = i - 1; j >= 0; j--) {
          const prevName = order[j];
          if (prevName === 'Sunrise') continue;
    ...
    if (nextFound && nextFoundDate) {
      setNextPrayer(nextFound);
      setCurrentPrayer(currentFound);
      const diff = nextFoundDate - now;
      setCountdown(
        `${String(Math.floor(diff / 3600000)).padStart(2, '0')}:` +
        `${String(Math.floor((diff % 3600000) / 60000)).padStart(2, '0')}:` +
        `${String(Math.floor((diff % 60000) / 1000)).padStart(2, '0')}`
  ...
  return (
    <>
          ...
          <View style={styles.timesCard}>
            ...
            {[
              ...
              <PrayerRow
                ...
                hasOffset={!!prayerOffsets[name] && prayerOffsets[name] !== 0}
      ...
      */}
      ...
      <DailyInsightModal
        visible={dailyInsightModalVisible}
        onClose={() => setDailyInsightModalVisible(false)}
      />
      <PrayerOffsetsModal
        visible={showOffsetsModal}
        onClose={() => setShowOffsetsModal(false)}
        theme={theme}
        offsets={prayerOffsets}
        onSave={handleSaveOffsets}
      />
    </>
  );
};
...
// ─── PRAYER ROW ─────────────────────────────────────────────────
const PrayerRow = ({ name, time, icon, theme, enabled, onToggle, t, hasOffset }) => (
  <View style={styles.prayerRow}>
    ...
    <View style={styles.prayerRight}>
      <Text style={[styles.prayerTime, { color: hasOffset ? '#e8a317' : theme.title }]}>{time}</Text>
