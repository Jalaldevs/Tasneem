  ...
  const { metadata, hadiths, sections } = useMemo(() => {
    ...
    const preparedSections = Object.entries(araData.metadata.sections)
      .map(([id, title]) => {
        ...
        if (id === "0" && selectedBook !== 'ibnmajah') return null;
        return { id, title: title || (id === "0" ? "المقدمة" : t('sunnahUI.sectionFallbackTitle', { id })), first: details.hadithnumber_first, last: details.hadithnumber_last };
      })
      .filter(Boolean);
